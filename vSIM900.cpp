...
VSIM900::VSIM900(HardwareSerial &hwPort, uint32_t baud, unsigned int switchPin, unsigned int resetPin) {
	VSIM900(hwPort, baud);
...
int VSIM900::sendATCommand(const String &cmd, unsigned int dWait, char retries,
		bool ignoreErrors) {
...
//  uint8_t i=0;
	...
	serialPort->write((unsigned char*) cmd, strlen(cmd));
	serialPort->write("\r\n");
	delay(100);
	readModemResponse();
	return (modem.status);
...
modemError VSIM900::initModem() {
	...
	if ((modem.status == msUNKNOWN) || (modem.status == msERROR)) {
		...
		this->debug(
				"init_failure_counter="
						+ String(modem.init_failure_counter, DEC));
		return (meINIT);
	} else {
...
//   _wdr();
	...
	if (modem.network_ok) {
		this->debug(F("NETWORK OK"));
	} else {
		this->debug(F("Net Not Ready"));
		return (meNETWORK);
	}
	blink(500, 1);
	blink(100, 1);
...
//  modem.gprs_ok = (modem.response.substring(8)=="1");
	...
	if (modem.gprs_ok) {
		this->debug(F("GPRS OK"));
	} else {
		this->debug(F("GPRS Failed"));
		return (meGPRS);
	}
	...
	if (i == msOK) {
		this->debug(F("+CIPRXGET: OK"));
	} else {
		this->debug(F("+CIPRXGET: FAIL?"));
	}
	counter = 0;
	...
	do {
		i = sendATCommand("AT+CSTT=\"" + String(_modemAPN) + "\"", 2000, 2,
				true);
		if (i == msOK) {
			counter = 3;
		} else
	...
	if (i == msOK) {
		this->debug(F("CIICR OK"));
	} else {
		this->debug(F("CIICR Failed?"));
		return (meCIICR);
	}
...
//    if (index >= BUFFER_SIZE) index = 0;   // gal geriau index=BUFFER_SIZE-1
		if (indx >= MODEM_BUFFER_SIZE)
			indx = MODEM_BUFFER_SIZE - 1;
...
modemState VSIM900::parseModemStatus(char *eilute) {
	...
	if (strcmp(eilute, "OK") == 0) {
		return msOK;
	} else if (strcmp(eilute, "ERROR") == 0) {
		return msERROR;
	} else if (strcmp(eilute, "RDY") == 0) {
		return msREADY;
	} else if (strcmp(eilute, "NO CARRIER") == 0) {
		return msNOCARIER;
	} else if (strcmp(eilute, "RING") == 0) {
		return msRING;
	} else if (strcmp(eilute, "CONNECT") == 0) {
		return msCONNECT;
	} else if (strcmp(eilute, "+CPIN: READY") == 0) {
		return msPIN_READY;
	} else if (strcmp(eilute, "SERVER OK") == 0) {
		return msSERVER_OK;
	} else if (strcmp(eilute, "SERVER CLOSE") == 0) {
		return msSERVER_CLOSE;
	} else if (strncmp(eilute, "+CIPRXGET:1", 11) == 0) {
		return msDATA_AVAILABLE;
	} else if (strncmp(eilute, "+CIPRXGET: 3", 12) == 0) {
		return msDATA_RECEIVING_HEX;
	} else if (strcmp(eilute, "+PDP: DEACT") == 0) {
		return msPDP_DEACT;
	} else if (strcmp(eilute, "NORMAL POWER DOWN") == 0) {
		return msPOWER_DOWN;
	} else if (strncmp(eilute, "REMOTE IP:", 10) == 0) {
		return msCLIENT_CONNECTED;
	} // TODO atskirt kai klientas prisijungia ir kai mes prisijungiam
	else if (strncmp(eilute, "CONNECT OK", 10) == 0) {
		return msCLIENT_CONNECTED;
	} else if (strncmp(eilute, "STATE: CONNECT OK", 17) == 0) {
		return msCLIENT_ONLINE;
	} else if (strncmp(eilute, "STATE: SERVER LIS", 17) == 0) {
		return msCLIENT_OFFLINE;
	} else if (strcmp(eilute, "CLOSED") == 0) {
		return msCLIENT_DISCONNECTED;
	} else if (strcmp(eilute, ">") == 0) {
		return msREADY_TO_SEND;
	} else if (strcmp(eilute, "SEND OK") == 0) {
		return msSEND_OK;
	} else if (strncmp(eilute, "DATA ACCEPT", 11) == 0) {
		return msDATA_ACCEPT;
	} else if (strncmp(eilute, "+DTMF:", 6) == 0) {
		return msDTMF_RECEIVED;
	} else if (strncmp(eilute, COMMAND_PREFIX, sizeof(COMMAND_PREFIX) - 1)
			== 0) {
		return msVALDIKLIS_PREFIX;
	} else if (strncmp(eilute, "+CSQ:", 5) == 0) {
		return msCSQ;
	} else if (strncmp(eilute, "+CMTE:", 6) == 0) {
		return msCMTE;
	} else if (strncmp(eilute, "SHUT OK", 7) == 0) {
		return msSHUT_OK;
	}
...
//		call DTMF callback if set
	...
	case msVALDIKLIS_PREFIX: {
		...
		if (specialCommand__ != nullptr)
			specialCommand__(eilute + 3, true);
		break;
	}
	case msCSQ: {
		int rssi;
		...
		this->debug(
				"+RSSI:" + String(rssi, DEC) + " BER:" + String(modem.ber, DEC)
						+ " ");
		break;
	}
	case msCMTE: {
...
void VSIM900::loop() {
	...
	if (ms - _idle_connection_data_check_time >= 10000) {
		if (((ms / 1000) - modem.last_dataReceivedTimestamp)
				> MODEM_MAX_GPRS_INACTIVIY_TIME) {
...
//	  sendATCommandChar(buffer);
		int s = snprintf_P(buffer, sizeof(buffer), PSTR("AT+CIPSEND=%d\r\n"),
				count);
		serialPort->write((unsigned char*) buffer, s);
		// not sure about these two lines
		delay(100);
//	  readModemResponse();
		serialPort->write((unsigned char*) dataToSend, count);
